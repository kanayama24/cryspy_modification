...
### `r_relax`（float）
- 最表面層からの距離（Å）を指定し、その範囲内の原子層を構造緩和の対象とする。

### `r_rearr`（float）
- 最表面層からの距離（Å）を指定し、その範囲内の原子層を再構成の対象とする。
- 対象原子層は一度除去され、ランダムに原子が再配置される。
- 原子数は `nat` により指定（任意の原子数を指定可能）。

### `r_above`（float）
- 最表面層から真空領域方向への距離（Å）を指定し、その範囲内に原子をランダムに配置する。
- すなわち、再構成層の原子は、最表面層から真空側に `r_above`、バルク側に `r_rearr` の範囲でランダムに配置される。

### `dual_surface`（bool）
- スラブ構造の両面を再構成層とみなすか（`dual_surface=True`）、片面のみとするか（`dual_surface=False`）を指定する。
